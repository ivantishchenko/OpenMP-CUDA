...
__global__ void medianKernel(float * output, const double * input, int ny, int nx) {
...
void correlate(int ny, int nx, const float* data, float* result) {
    ...
    double * input_CPU = nullptr;
   // double * output_CPU = nullptr;  
    double * input_GPU = nullptr;  
  ...
  //  if (res_in_host == cudaSuccess && res_out_host == cudaSuccess ) std::cout << "Allocated host memory" << std::endl;
   // if (res_in_dev == cudaSuccess && res_out_dev == cudaSuccess ) std::cout << "Allocated host memory" << std::endl;
    ...
    for ( int i = 0; i < ny; i++ ) {
        ...
        // CPU -> GPU
    cudaMemcpy(input_GPU, input_CPU, N * sizeof(double), cudaMemcpyHostToDevice);
